...
impl InnerPlayer {
    ...
    /// 启动 FFT 推送定时器（独立线程，每 32ms 推送一次频谱数据，约 30Hz）
    /// 30Hz 与渲染端 IslandSpectrum 的 30fps 节流对齐，减半 IPC 与 CPU 开销
    /// 线程内部已假设 fft_enabled=true（调用方负责确保），无消费者时由 set_fft_enabled 关闭定时器
    fn start_fft_timer(&mut self) {
        ...
        let handle = thread::spawn(move || {
            while !stop_flag.load(Ordering::Relaxed) {
                ...
                sleep_unless_stopped(&stop_flag, Duration::from_millis(32));
            }
        });
    ...
    /// 设置 FFT 推送开关
    /// 无消费者时彻底关闭定时器线程，避免 32ms 空轮询；有消费者时按需启动
    pub fn set_fft_enabled(&mut self, enabled: bool) {
        let prev = self.fft_enabled.swap(enabled, Ordering::Relaxed);
        if enabled == prev {
            return;
        }
        // 仅在播放态响应开关：暂停/停止态定时器本就停了，恢复播放时按当前 fft_enabled 决定是否启动
    ...
    /// 设置频段数量（最多 32）
    pub fn set_equalizer_band_count(&mut self, count: usize) {
        self.equalizer.lock().set_band_count(count);
    }
    ...
    pub fn set_equalizer_band_params(
        ...
        self.equalizer
            .lock()
            .set_band_params(index, freq, q, gain_db, filter_type)
    }

    /// 获取所有频段参数
    pub fn equalizer_band_params(&self) -> Vec<BandParams> {
        self.equalizer.lock().band_params()
    }

    /// 计算指定频率列表上的总频响（dB）
    pub fn equalizer_frequency_response_db(&self, freqs: &[f32]) -> Vec<f32> {
        self.equalizer.lock().frequency_response_db(freqs)
    }

    /// 取输入电平 RMS（左/右声道，0~1 线性）
    pub fn equalizer_input_levels(&self) -> [f32; 2] {
        self.equalizer.lock().input_levels()
    }

    /// 取输出电平 RMS（左/右声道，0~1 线性）
    pub fn equalizer_output_levels(&self) -> [f32; 2] {
        self.equalizer.lock().output_levels()
    }

    /// 设置 A/B bypass（true 时跳过滤波）
    pub fn set_equalizer_bypass(&mut self, bypass: bool) {
        self.equalizer.lock().set_bypass(bypass);
    }

    /// 获取 bypass 状态
    pub fn equalizer_bypass(&self) -> bool {
        self.equalizer.lock().bypass()
    }
    ...
    /// 设置低音增益（dB，自动 clamp 到 ±12）
    pub fn set_bass_gain(&mut self, db: f32) {
        self.equalizer.lock().set_bass_gain_db(db);
    }

    /// 获取低音增益（dB）
    pub fn bass_gain(&self) -> f32 {
        self.equalizer.lock().bass_gain_db()
    }

    /// 设置高音增益（dB，自动 clamp 到 ±12）
    pub fn set_treble_gain(&mut self, db: f32) {
        self.equalizer.lock().set_treble_gain_db(db);
    }

    /// 获取高音增益（dB）
    pub fn treble_gain(&self) -> f32 {
        self.equalizer.lock().treble_gain_db()
    }

    /// 设置环绕声增益（1.0 = 原始，>1 扩展立体声场）
    pub fn set_surround_gain(&mut self, gain: f32) {
        self.equalizer.lock().set_surround_gain(gain);
    }

    /// 获取环绕声增益
    pub fn surround_gain(&self) -> f32 {
        self.equalizer.lock().surround_gain()
    }
    ...
    /// 获取音频超分当前生效后端
    pub fn super_resolution_effective_backend(&self) -> SuperResBackend {
        self.super_res.effective_backend()
    }
