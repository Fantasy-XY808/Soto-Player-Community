...
impl From<crate::equalizer::BandParams> for JsBandParams {
    fn from(params: crate::equalizer::BandParams) -> Self {
        Self {
            freq: params.freq as f64,
            q: params.q as f64,
            gain_db: params.gain_db as f64,
            filter_type: params.filter_type.to_u8(),
        }
    }
}
...
impl AudioPlayer {
    ...
    /// 设置频段数量（最多 32）
    #[napi]
    pub fn set_equalizer_band_count(&self, count: u32) {
        self.inner.lock().set_equalizer_band_count(count as usize);
    }
    ...
    #[napi]
    pub fn set_equalizer_band_params(
        &self,
        index: u32,
        freq: f64,
        q: f64,
        ...
        self.inner.lock().set_equalizer_band_params(
            index as usize,
            freq as f32,
            q as f32,
            gain_db as f32,
            crate::equalizer::FilterType::from_u8(filter_type),
        )
    }

    /// 获取所有频段参数
    #[napi]
    pub fn get_equalizer_band_params(&self) -> Vec<JsBandParams> {
        self.inner
            .lock()
            .equalizer_band_params()
            .into_iter()
            .map(JsBandParams::from)
            .collect()
    }
    ...
    #[napi]
    pub fn get_equalizer_frequency_response(&self, freqs: Vec<f64>) -> Vec<f64> {
        let freqs_f32: Vec<f32> = freqs.into_iter().map(|v| v as f32).collect();
        self.inner
            .lock()
            .equalizer_frequency_response_db(&freqs_f32)
            .into_iter()
            .map(|v| v as f64)
            .collect()
    }
    ...
    #[napi]
    pub fn get_equalizer_input_levels(&self) -> Vec<f64> {
        let levels = self.inner.lock().equalizer_input_levels();
        levels.iter().map(|v| *v as f64).collect()
    }

    /// 取输出电平 RMS（左/右声道，0~1 线性）
    #[napi]
    pub fn get_equalizer_output_levels(&self) -> Vec<f64> {
        let levels = self.inner.lock().equalizer_output_levels();
        levels.iter().map(|v| *v as f64).collect()
    }

    /// 设置 A/B bypass（true 时跳过滤波）
    #[napi]
    pub fn set_equalizer_bypass(&self, bypass: bool) {
        self.inner.lock().set_equalizer_bypass(bypass);
    }

    /// 获取 bypass 状态
    #[napi]
    pub fn get_equalizer_bypass(&self) -> bool {
        self.inner.lock().equalizer_bypass()
    }
    ...
    #[napi]
    pub fn set_bass_gain(&self, bass_db: f64) {
        self.inner.lock().set_bass_gain(bass_db as f32);
    }

    /// 获取低音增益（dB）
    #[napi]
    pub fn get_bass_gain(&self) -> f64 {
        self.inner.lock().bass_gain() as f64
    }

    /// 设置高音增益（dB），范围 [-12, 12]，正值增强高频
    #[napi]
    pub fn set_treble_gain(&self, treble_db: f64) {
        self.inner.lock().set_treble_gain(treble_db as f32);
    }

    /// 获取高音增益（dB）
    #[napi]
    pub fn get_treble_gain(&self) -> f64 {
        self.inner.lock().treble_gain() as f64
    }

    /// 设置环绕声增益（1.0 = 原始，>1 扩展立体声场，0~3）
    #[napi]
    pub fn set_surround_gain(&self, gain: f64) {
        self.inner.lock().set_surround_gain(gain as f32);
    }

    /// 获取环绕声增益
    #[napi]
    pub fn get_surround_gain(&self) -> f64 {
        self.inner.lock().surround_gain() as f64
    }
    ...
    #[napi]
    pub fn get_audio_super_resolution_effective_backend(&self) -> u8 {
        self.inner.lock().super_resolution_effective_backend().to_u8()
    }
