...
/// 解析 `Content-Range: bytes {start}-{end}/{total}` 中的 start 字段
/// 用于校验 206 响应确实是从本地 pos 开始的，防止服务端静默忽略 Range 导致数据错位
fn parse_content_range_start(header: &str) -> Option<u64> {
    let after_bytes = header.rsplit_once("bytes ")?.1.trim();
    let start_str = after_bytes.split_once('-')?.0.trim();
    start_str.parse().ok()
}
    ...
    fn open_stream(&mut self) -> OpenOutcome {
        ...
        match self.agent.get(&self.url).set("Range", &range_header).call() {
            Ok(resp) => {
                ...
                // 校验 206 响应的 Content-Range 起点 == 本地 pos
                // 部分 CDN 在 URL 失效后返回从 0 开始的 206，不做校验会让数据"看起来正常"地写入错误位置
                if status == 206 && self.pos > 0 {
                    if let Some(cr) = resp.header("Content-Range") {
                        match parse_content_range_start(cr) {
                            Some(start) if start == self.pos => {}
                            Some(start) => {
                                return OpenOutcome::Fatal(io::Error::other(format!(
                                    "Content-Range 起点 {start} 与请求 pos {} 不一致，CDN 返回了错位数据",
                                    self.pos
                                )));
                            }
                            None => {
                                warn!(cr, "206 响应的 Content-Range 无法解析起点，按 best-effort 继续");
                            }
                        }
                    }
                }
...
mod tests {
    ...
    #[test]
    fn parse_content_range_start_basic() {
        assert_eq!(parse_content_range_start("bytes 0-999/12345"), Some(0));
        assert_eq!(parse_content_range_start("bytes 2000-3999/8192"), Some(2000));
