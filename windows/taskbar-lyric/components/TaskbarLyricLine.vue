...
/** 是否需要 RAF：仅 karaoke 逐字或内容溢出滚动时 */
const needsRaf = (): boolean => useKaraoke.value || overflowPx.value > 0;

const renderFrame = (): void => {
  if (!needsRaf() || document.hidden) {
    rafId = 0;
    return;
  }
...
const startRenderLoop = (): void => {
  if (rafId === 0 && needsRaf() && !document.hidden) {
    rafId = requestAnimationFrame(renderFrame);
  }
};
...
watch(
  () => [useKaraoke.value, overflowPx.value],
  () => {
    if (needsRaf()) {
      startRenderLoop();
    } else {
