...
const displayItems = computed<DisplayItem[]>(() => {
  ...
  // 无曲目
  if (!cur) return placeholder("ph-idle", "Soto Player Desktop Lyric");
  // 占位歌曲信息
...
onMounted(() => {
  // 先注册监听器，避免主进程首次下发的事件被丢弃（与 taskbar-lyric 同样的竞态）
  unsubConfig = window.api.desktopLyric.onConfigChange((next) => Object.assign(config, next));

  void window.api.config
    .get("desktopLyric")
    .then((saved) => {
      if (saved) Object.assign(config, saved as DesktopLyricSettings);
    })
    .catch((error) => {
  ...
  >
    ...
    <div class="header">
      <div class="header-section header-left">
        ...
        <div class="song-info">
          <div class="song-title">{{ track?.title ?? "Soto Player Desktop Lyric" }}</div>
