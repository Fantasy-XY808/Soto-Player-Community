...
const expandedAlpha = computed(() => Math.max(0, Math.min(1, (progress.value - 0.2) / 0.5)));
...
const { onContentPointerDown } = useDragWindow({
  enabled: () => !isExpanded.value && !notchFusionEnabled.value && !config.nonOcclusive,
...
const maxLyricSlotWidth = computed(() => {
  /* 固定宽度模式：slot = fixedWidth - 固定内容区，超出由 overflowMode 处理 */
...
/* 计算窗口宽度：fixed 模式直接返回 fixedWidth，adaptive 模式按内容 + 回弹余量 */
...
/* 宽度模式/固定宽度/最大宽度/超出处理变化：重算 slot 与窗口宽度 */
watch(
  [
    () => config.widthMode,
    () => config.fixedWidth,
...
/* 文本是否超出 slot：truncate 模式下由截断结果判断，scroll 模式下测量原始宽度 */
...
const fittedDisplayLine = computed<LyricLine | null>(() => {
  ...
  /* scroll 模式下文本超出时禁用逐字高亮，统一走 marquee 滚动 */
...
<template>
  ...
  >
    ...
    />
    <!-- 点击空白区域收起：动画期间保持挂载 -->
    <div v-if="isExpanded || progress > 0.001" class="collapse-overlay" @click.stop="collapse" />
  </div>
</template>
