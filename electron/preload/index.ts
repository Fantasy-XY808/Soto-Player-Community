...
const api = {
  ...
  player: {
    ...
    // 更新均衡器各频段增益（dB 数组，长度 10）—— 兼容旧接口
    setEqualizerBands: (gainsDb: number[]) =>
      ipcRenderer.invoke("player:setEqualizerBands", gainsDb),
    // 设置均衡器频段数量
    setEqualizerBandCount: (count: number) =>
      ipcRenderer.invoke("player:setEqualizerBandCount", count),
    // 设置指定频段的完整参数（频率 / Q / 增益 / 滤波器类型）
    setEqualizerBandParams: (
      index: number,
      params: { freq: number; q: number; gain: number; filterType: number },
    ) =>
      ipcRenderer.invoke(
    ...
    // 取所有频段当前参数
    getEqualizerBandParams: () => ipcRenderer.invoke("player:getEqualizerBandParams"),
    // 取频响曲线在指定频率点上的总增益（dB）
    getEqualizerFrequencyResponse: (freqs: number[]) =>
      ipcRenderer.invoke("player:getEqualizerFrequencyResponse", freqs),
    // 取输入电平 RMS（左/右声道，0~1 线性）
    getEqualizerInputLevels: () => ipcRenderer.invoke("player:getEqualizerInputLevels"),
    // 取输出电平 RMS（左/右声道，0~1 线性）
    getEqualizerOutputLevels: () => ipcRenderer.invoke("player:getEqualizerOutputLevels"),
    // 设置 A/B bypass（true = 跳过滤波器，用于效果对比）
    setEqualizerBypass: (bypass: boolean) =>
      ipcRenderer.invoke("player:setEqualizerBypass", bypass),
    // 取当前 bypass 状态
    getEqualizerBypass: () => ipcRenderer.invoke("player:getEqualizerBypass"),
    // 设置前级增益（dB）
    setPreampGain: (preampDb: number) => ipcRenderer.invoke("player:setPreampGain", preampDb),
    // 设置低音增益（dB，200Hz 以下 low-shelf）
    setBassGain: (bassDb: number) => ipcRenderer.invoke("player:setBassGain", bassDb),
    // 设置高音增益（dB，3500Hz 以上 high-shelf）
    setTrebleGain: (trebleDb: number) => ipcRenderer.invoke("player:setTrebleGain", trebleDb),
  ...
  migration: {
    ...
    hasLegacyData: () => ipcRenderer.invoke("migration:hasLegacyData"),
    // 执行迁移：覆盖当前数据（用户须先确认）
    perform: (): Promise<{ ok: boolean; migrated: string[]; error?: string }> =>
      ipcRenderer.invoke("migration:perform"),
  },
  cloud: {
    // 弹出文件选择器选择待上传歌曲
  ...
  listenTogether: {
    // 拉取当前会话状态
    getStatus: (): Promise<ListenTogetherStatus> => ipcRenderer.invoke("listenTogether:getStatus"),
    // 订阅会话状态变化
    onStatus: (callback: (status: ListenTogetherStatus) => void): (() => void) => {
      const handler = (_event: Electron.IpcRendererEvent, data: ListenTogetherStatus): void =>
        callback(data);
      ipcRenderer.on("listenTogether:status", handler);
      ipcRenderer.send("listenTogether:onStatus");
      return () => {
        ipcRenderer.removeListener("listenTogether:status", handler);
        ipcRenderer.send("listenTogether:offStatus");
      };
    },
    // 查询本地网易云登录态（启动前自检）
    getLocalUserInfo: (): Promise<ListenTogetherLocalUser | null> =>
      ipcRenderer.invoke("listenTogether:getLocalUserInfo"),
    // 启动主机模式
    startHost: (
      name: string,
      password: string,
    ...
    // 开始浏览局域网会话
    browseSessions: (): void => {
      ipcRenderer.send("listenTogether:browseSessions");
    },
    // 停止浏览
    stopBrowse: (): void => {
      ipcRenderer.send("listenTogether:stopBrowse");
    },
    // 订阅会话发现结果
    onDiscovered: (
      ...
      const handler = (
        ...
        data: ListenTogetherDiscoveredSession[],
      ): void => callback(data);
      ipcRenderer.on("listenTogether:discovered", handler);
      return () => {
        ipcRenderer.removeListener("listenTogether:discovered", handler);
      };
    },
    // 一次性查询当前已发现的会话
  ...
  stats: {
    ...
    // 取 24 小时播放分布
    getHourlyDistribution: () => ipcRenderer.invoke("stats:getHourlyDistribution"),
  },
  hotkey: {
   ...
   */
  easterEgg: {
    show: (): Promise<void> => ipcRenderer.invoke("easter-egg:show"),
    close: (): Promise<void> => ipcRenderer.invoke("easter-egg:close"),
    available: (): Promise<boolean> => ipcRenderer.invoke("easter-egg:available"),
    onShowHint: (callback: (text: string) => void) => {
      const listener = (_event: unknown, text: string): void => callback(text);
      ipcRenderer.on("easter-egg:show-hint", listener);
      return () => ipcRenderer.off("easter-egg:show-hint", listener);
    },
  },
};
