...
export const getTopArtists = (limit: number): TopArtist[] => {
  try {
    const rows = getDb()
      .prepare(
        `SELECT track_json, COUNT(*) AS plays, COALESCE(SUM(listened_ms), 0) AS ms
         FROM play_history
         GROUP BY source, track_id
         ORDER BY plays DESC
         LIMIT 500`,
...
/**
 * 取 24 小时播放次数分布
 * 用 SQLite 的 strftime 取本地时区小时,直接 GROUP BY
 */
export const getHourlyDistribution = (): HourlyStat[] => {
  try {
    const rows = getDb()
      .prepare(
    ...
    const map = new Map<number, number>(rows.map((r) => [r.hour, r.plays]));
    return Array.from({ length: 24 }, (_, h) => ({
      hour: h,
      playCount: map.get(h) ?? 0,
    }));
  } catch (error) {
    libraryLog.error("读取小时分布失败:", error);
    return Array.from({ length: 24 }, (_, h) => ({ hour: h, playCount: 0 }));
  }
};
