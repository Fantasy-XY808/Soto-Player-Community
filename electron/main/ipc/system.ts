...
export const registerSystemIpc = (): void => {
  ...
  // 获取系统已安装字体
  // font-list 在 Windows 上通过 PowerShell 子进程枚举字体，部分环境下会 hang（执行策略受限、
  // 字体目录过大、PowerShell 模块加载慢），且包内无超时——hang 时 Promise 永不 resolve，
  // fontsCache 持有 pending Promise 导致后续调用复用同一个 pending，UI 永远显示"加载中"。
  // 这里用 8s 超时竞速兜底，超时返回空数组并清缓存允许下次重试
