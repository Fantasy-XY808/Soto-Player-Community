 ...
 *
 * 与 Qobuz 之不同：
 * - mora 是付费 Hi-Res 商店，未公开 API 文档；真实端点需逆向工程获得
 * - 真实取流需附带 X-Mora-Device-Id + session token，匿名调用拿不到结果
 * - 阶段 1 scaffold：search / song_url 模块直接返回空结果 + 明确日志
 *
 * 统一入口：callMora(name, params)
 *
...
import type { MoraParams } from "./core/types";

/** 2 分钟响应缓存（与 Qobuz / archive 一致） */
...
export const clearMoraCache = (): void => {
  cache.clear();
};
 ...
 */
export const callMora = async (name: string, params: MoraParams = {}): Promise<any> => {
  const fn = Object.hasOwn(modules, name) ? modules[name] : undefined;
  if (!fn) throw new Error(`unknown mora api: ${name}`);
