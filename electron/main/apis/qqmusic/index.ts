...
/**
 * 判定 song_url 结果是否为"空 URL"——VIP/版权受限歌曲返回 { code: 200, url: "" }
 * 这类结果不应缓存，否则 2 分钟内重试永远命中空结果，无法立即回落插件
 */
const isEmptyUrlResult = (name: string, value: unknown): boolean => {
  if (name !== "song_url" || !value || typeof value !== "object") return false;
  const o = value as Record<string, unknown>;
  return o.code === 200 && (o.url === "" || o.url === null || o.url === undefined);
};
...
export const callQQMusic = async (name: string, params: QMParams = {}): Promise<any> => {
  ...
  // 空 URL 结果不缓存：避免 2 分钟内重试命中空结果，无法立即回落插件源
  if (!isEmptyUrlResult(name, value)) {
    cacheSet(key, value);
  }
  return value;
};
