/**
 * ProStudioMasters 主进程服务
 *
 * 与 Qobuz 之不同：
 * - ProStudioMasters 是付费 Hi-Res 商店，未公开 API 文档；真实端点需逆向工程获得
 * - 真实取流需附带 session token / cookie，匿名调用拿不到结果
 * - 阶段 1 scaffold：search / song_url 模块直接返回空结果 + 明确日志
 *
 * 统一入口：callPsm(name, params)
 *
...
import type { PsmParams } from "./core/types";

/** 2 分钟响应缓存（与 Qobuz / archive / mora 一致） */
...
export const clearPsmCache = (): void => {
  cache.clear();
};

/**
 * 调用任意 ProStudioMasters API
 * @param name    见 modules/index.ts（search / song_url / lyric）
 * @param params  业务参数；不想命中缓存可传 `timestamp: Date.now()`
 */
export const callPsm = async (name: string, params: PsmParams = {}): Promise<any> => {
  const fn = Object.hasOwn(modules, name) ? modules[name] : undefined;
  if (!fn) throw new Error(`unknown prostudiomasters api: ${name}`);
