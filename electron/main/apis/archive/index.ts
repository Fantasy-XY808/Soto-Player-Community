/**
 * Internet Archive 主进程服务
 *
 * 与 Qobuz 之不同：
 * - 完全无鉴权：不需要 token / cookie / signature
 * - 公开 API：advancedsearch.php + /metadata/{id} + /download/{id}/{file}
 * - 海外平台：必须走 overseasFetch（注入代理 dispatcher）
 *
 * 统一入口：callArchive(name, params)
 */

import { createHash } from "node:crypto";
import { modules } from "./modules";
import type { ArchiveParams } from "./core/types";
 ...
 */
export const callArchive = async (name: string, params: ArchiveParams = {}): Promise<any> => {
  const fn = Object.hasOwn(modules, name) ? modules[name] : undefined;
  if (!fn) throw new Error(`unknown archive api: ${name}`);
