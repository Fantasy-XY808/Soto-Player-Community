 ...
 * - 2L 仅提供试听曲目（非付费商店），仅用于个人试听模式，禁止曲库收录
 * - search / song_url 模块直接返回空结果 + 明确日志
 * - 后续接入真实端点时，UI 必须标注「试听曲目」且禁止加入用户曲库
 *
 * 统一入口：callTwoL(name, params)
 *
 * 错误码段位：14XXX（mora 占 14XXX-A/B，2L 占 14XXX-C/D/E）
 */

import { createHash } from "node:crypto";
import { modules } from "./modules";
import type { TwoLParams } from "./core/types";
...
export const clearTwoLCache = (): void => {
  cache.clear();
};
 ...
 */
export const callTwoL = async (name: string, params: TwoLParams = {}): Promise<any> => {
  const fn = Object.hasOwn(modules, name) ? modules[name] : undefined;
  if (!fn) throw new Error(`unknown 2L api: ${name}`);
