 ...
 * - 搜索需要 buvid3 cookie（匿名占位值即可），在 core/request.ts 统一注入
 * - 视频元数据来自 /x/web-interface/view，音频流来自 /x/player/playurl（DASH 格式）
 * - 默认非真母带：UP 主上传的音频来源不定，多为转码后的有损/16bit FLAC
 *
 * 统一入口：callBilibili(name, params)
 */

import { createHash } from "node:crypto";
import { modules } from "./modules";
import type { BiliParams } from "./core/types";

/** 2 分钟响应缓存（与 archive / Qobuz 一致） */
...
export const clearBilibiliCache = (): void => {
  cache.clear();
};
 ...
 */
export const callBilibili = async (name: string, params: BiliParams = {}): Promise<any> => {
  const fn = Object.hasOwn(modules, name) ? modules[name] : undefined;
  if (!fn) throw new Error(`unknown bilibili api: ${name}`);
