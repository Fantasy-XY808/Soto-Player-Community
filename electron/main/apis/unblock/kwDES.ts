...
/** Long 对象接口：用 BigInt 实现 64 位整数运算 */
interface LongLike {
  /** 低 32 位 */
  low: number;
  valueOf(): bigint;
...
/** 把 LongLike / number / bigint 统一转 bigint；LongLike 走 valueOf() */
const toBigInt = (x: LongLike | number | bigint): bigint =>
  typeof x === "object" ? x.valueOf() : BigInt(x);
...
const Long = (n: number | bigint | LongLike): LongLike => {
  ...
  return {
    ...
    not: () => Long(~bN),
    isNegative: () => bN < 0n,
    or: (x) => Long(bN | toBigInt(x)),
    and: (x) => Long(bN & toBigInt(x)),
    xor: (x) => Long(bN ^ toBigInt(x)),
    equals: (x) => bN === toBigInt(x),
    multiply: (x) => Long(bN * toBigInt(x)),
    shiftLeft: (x) => Long(bN << BigInt(x)),
    shiftRight: (x) => Long(bN >> BigInt(x)),
  };
};

const range = (n: number): number[] => Array.from(new Array(n).keys());

const power = (base: LongLike, index: number): LongLike =>
  Array(index)
    .fill(null)
    .reduce((result) => result.multiply(base), Long(1));

const LongArray = (...array: number[]): LongLike[] =>
...
const arrayMask = range(64).map((n) => power(Long(2), n));
...
const DES64 = (longs: LongLike[], l: LongLike): LongLike => {
  ...
  const pSource: LongLike[] = [Long(0), Long(0)];
  let L = Long(0);
  let R = Long(0);
...
const subKeys = (l: LongLike, longs: LongLike[], n: number): void => {
...
const crypt = (msg: Buffer, key: Buffer, mode: number): Buffer => {
  // 处理密钥块
  let l = Long(0);
  range(8).forEach((i) => {
    l = Long(key[i])
  ...
  const arrByte1 = msg.subarray(j * 8);
  let l2 = Long(0);
 ...
 * @returns 加密后的字节
 */
export const encrypt = (msg: Buffer): Buffer => crypt(msg, SECRET_KEY, 0);
 ...
 * @returns 解密后的字节
 */
export const decrypt = (msg: Buffer): Buffer => crypt(msg, SECRET_KEY, 1);
...
export const encryptQuery = (query: string): string =>
  encrypt(Buffer.from(query)).toString("base64");
