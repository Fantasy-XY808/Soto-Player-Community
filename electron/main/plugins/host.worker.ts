 ...
 *
 * 与 sandbox.worker.ts 的差异：
 * - sandbox.worker.ts：一个 utilityProcess 托管一个插件（per-plugin 模型）
 * - host.worker.ts：一个 utilityProcess 托管所有已启用插件（multi-plugin 模型）
 *   每插件一个 node:vm 上下文（按 pluginId 索引），共享一条 parentPort 与主进程通信
 *
 * 协议：
 ...
 *
 * 集成状态：保守策略，仅新增文件不接入主流程，留作未来切换 multi-plugin host 模式时使用。
 * 当前由 sandbox.ts + sandbox.worker.ts 提供 per-plugin 隔离，已满足生产需求。
 *
...
/** host ↔ main 私有消息协议（与 sandbox.worker.ts 的 SandboxIn/SandboxOut 独立） */
type HostMessageIn =
  | {
      kind: "loadPlugin";
      pluginId: string;
  ...
  | { kind: "event"; pluginId: string; event: string; data: unknown }
...
type HostMessageOut =
  ...
  | { kind: "fatal"; pluginId: string; error: { code: string; message: string } }
  ...
  | { kind: "updateAvailable"; pluginId: string; info: { version?: string; log?: string; updateUrl?: string; updatedAt: number } };

interface ParentPort {
  on: (evt: "message", cb: (event: { data: HostMessageIn }) => void) => void;
  postMessage: (msg: HostMessageOut) => void;
}

const parentPort: ParentPort | undefined = (
  process as unknown as { parentPort?: ParentPort }
).parentPort;
 ...
 * @param msg - 待发消息
 */
const send = (msg: HostMessageOut): void => {
  try {
    parentPort!.postMessage(msg);
    return;
  } catch {
    try {
      parentPort!.postMessage(sanitizeForIpc(msg) as HostMessageOut);
      return;
    } catch (err2) {
      throw new Error(
...
const hostCall = (
  ...
  return new Promise<unknown>((resolve, reject) => {
    ...
    try {
      ...
      reject(Object.assign(new Error((err as Error).message), { code: "PLUGIN_ARGS_NOT_CLONEABLE" }));
    }
  });
};
...
type LoadSpec = Extract<HostMessageIn, { kind: "loadPlugin" }>;
...
const buildSplayer = (record: PluginContextRecord, spec: LoadSpec): HostApi => ({
  ...
  player: {
    ...
    seek: (positionMs: number) => void hostCall(record, "player.seek", [positionMs]).catch(() => {}),
    setVolume: (volume: number) => void hostCall(record, "player.setVolume", [volume]).catch(() => {}),
...
const buildUtils = (): object => ({
  crypto: {
    ...
    hmac: (algo: string, key: string | Uint8Array, data: string | Uint8Array) =>
      crypto.createHmac(algo, key as crypto.BinaryLike).update(data as crypto.BinaryLike).digest("hex"),
...
const loadPluginIntoContext = (spec: LoadSpec): void => {
  ...
  (splayer as unknown as { utils: object }).utils = buildUtils();
      ...
      case "call": {
        ...
        try {
          ...
          send({
            ...
            error: {
              code:
                (err as Error & { code?: string })?.code ?? "PLUGIN_HANDLER_ERROR",
