 ...
 *
 * 与 sandbox.ts 的差异：
 * - sandbox.ts：每插件一个 utilityProcess（per-plugin 模型）
 * - host-process.ts：单例 utilityProcess 托管所有插件（multi-plugin 模型）
 *
 * 集成状态：保守策略，仅新增文件不接入主流程，留作未来切换 multi-plugin host 模式时使用。
 * 当前由 sandbox.ts 提供 per-plugin 隔离，已满足生产需求；本文件提供 multi-plugin 模式
 * 的可选实现，可在未来通过 plugins/index.ts 或 router.ts 中的配置开关切换。
 *
 * 关键设计：
 ...
 *
 * 注意：HostMessageIn/Out 与 host.worker.ts 内的定义保持一致（重复定义避免从 worker 入口
 * 文件 import 触发其 process.exit 副作用）
 */
...
/** host ↔ main 私有消息协议（与 host.worker.ts 内定义镜像） */
type HostMessageIn =
  | {
      kind: "loadPlugin";
      pluginId: string;
...
export interface PluginHostCallbacks {
  ...
  onRegistered?: (events: PlaybackEventKind[], controls: boolean, settings: PluginSettingItem[]) => void;
  /** host 进程整体丢失（崩溃/卡死），此插件随之失效，需上层安排整体重启 */
  onHostLost: () => void;
}

/** loadPlugin 入参（即 loadPlugin 消息去掉 kind） */
export type PluginLoadSpec = Omit<Extract<HostMessageIn, { kind: "loadPlugin" }>, "kind">;

const resolveWorkerEntry = (): string => path.join(app.getAppPath(), "out", "main", "host.worker.js");
...
class PluginHost {
  ...
  ensureStarted(): Promise<void> {
    ...
    this.startPromise = new Promise<void>((resolve, reject) => {
      ...
      child.on("message", (msg: HostMessageOut) => {
  ...
  private post(msg: HostMessageIn): void {
  ...
  private onMessage(msg: HostMessageOut): void {
    switch (msg.kind) {
      case "hostReady": {
        this.hostReady = true;
        if (this.startTimer) {
      ...
      case "registered":
        this.callbacks
          .get(msg.pluginId)
          ?.onRegistered?.(msg.events, msg.controls, msg.settings);
        return;
      case "log":
  ...
  private startHeartbeat(): void {
    ...
    this.heartbeatTimer = setInterval(() => {
      ...
      try {
        this.child.postMessage({ kind: "ping" } satisfies HostMessageIn);
      } catch {
        /* ignore */
      }
    }, HEARTBEAT_INTERVAL);
