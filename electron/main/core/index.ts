 ...
 * - 禁止预热备用渲染进程：减少常驻渲染进程数
 * - 限制渲染进程 V8 老生代堆：触发更积极 GC，降低内存峰值
 * - 禁用 BackForwardCache：避免缓存不可见页面，减少内存
 */
...
export const initApp = (): void => {
  ...
  // 其他初始化
  app.whenReady().then(() => {
    electronApp.setAppUserModelId("top.fantasy-xy808.soto-player-community");
    // 注册 cache:// 协议处理
    handleCacheProtocol();
