...
export const applyDesktopLyricLock = (locked: boolean): void => {
  ...
  // 锁定模式停止光标轮询，解锁模式启动
  if (locked) stopCursorPolling();
  else startCursorPolling();
};
...
export const createDesktopLyricWindow = (): BrowserWindow => {
  ...
  desktopLyricWindow.once("ready-to-show", () => {
    ...
    // 锁定模式下窗口忽略鼠标，不需要光标轮询
    if (!config.locked) startCursorPolling();
    // 主动推一次配置，确保渲染进程收到最新值（避免监听器注册时机竞态）
    desktopLyricWindow.webContents.send("desktopLyric:configChange", config);
  });
