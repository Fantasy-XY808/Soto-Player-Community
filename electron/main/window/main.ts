...
export const createMainWindow = (): BrowserWindow => {
  ...
  // 窗口不可见时主动触发渲染进程 GC，降低后台内存占用
  // --expose-gc 由 configureMemoryOptimizations 开启，window.gc 不存在时静默跳过
  const triggerRendererGc = (): void => {
    if (!mainWindow || mainWindow.isDestroyed()) return;
    void mainWindow.webContents
      .executeJavaScript("if (typeof window.gc === 'function') window.gc();")
      .catch(() => {});
  };
  mainWindow.on("minimize", triggerRendererGc);
  mainWindow.on("hide", triggerRendererGc);
  // 最大化
