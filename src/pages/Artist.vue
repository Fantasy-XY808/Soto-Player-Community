...
import { fetchSimiArtists } from "@/apis/discover/netease";
...
const ARTIST_TAB_KEYS: readonly ArtistTab[] = ["songs", "albums", "similar"];
...
watch(activeTab, (tab) => {
  if (tab === "albums" || tab === "similar") collapsed.value = true;
  if (tab === "similar") void loadSimiArtists();
});
...
const tabs = computed(() => {
  ...
  if (source === "netease") {
    items.push({ key: "similar", label: t("artist.similar") });
  }
  return items;
});
...
/** 相似歌手 */
const simiArtists = shallowRef<CoverItem[]>([]);
const simiLoading = ref(false);
const simiLoaded = ref(false);

/** 加载相似歌手 */
const loadSimiArtists = async (): Promise<void> => {
  if (source !== "netease" || simiLoaded.value || simiLoading.value) return;
  simiLoading.value = true;
  try {
    simiArtists.value = await fetchSimiArtists(decodeURIComponent(id));
    simiLoaded.value = true;
  } catch (error) {
    console.warn("[artist] simi artists failed:", error);
  } finally {
...
/** 跳转到相似歌手 */
const openSimiArtist = (item: CoverItem): void => {
    ...
    <Transition name="fade" mode="out-in" :duration="150">
      ...
      >
        <Transition :name="tabTransitionName" mode="out-in">
          ...
          <!-- 相似歌手 -->
          <div v-else-if="activeTab === 'similar'" key="similar" class="flex-1 min-h-0">
            <div v-if="simiLoading" class="flex items-center justify-center py-20">
