...
import { fetchSimiPlaylists } from "@/apis/discover/netease";
...
/** 是否可显示相似歌单入口（仅网易云歌单） */
const canShowSimi = computed(() => source === "netease" && type === "playlist");
/** 相似歌单弹窗开关 */
const simiOpen = ref(false);
/** 相似歌单列表 */
const simiPlaylists = shallowRef<CoverItem[]>([]);
/** 相似歌单加载中 */
const simiLoading = ref(false);

/** 打开相似歌单弹窗并拉取数据 */
const openSimi = async (): Promise<void> => {
  simiOpen.value = true;
  if (simiPlaylists.value.length > 0) return;
  simiLoading.value = true;
  try {
    simiPlaylists.value = await fetchSimiPlaylists(id);
  } catch (error) {
    console.warn("[collection] simi playlists failed:", error);
  } finally {
...
/** 跳转到相似歌单 */
const openSimiPlaylist = (item: CoverItem): void => {
  simiOpen.value = false;
...
<template>
  <div class="flex flex-col h-full">
    ...
    <!-- 相似歌单 -->
    <SDialog v-model:open="simiOpen" :title="t('collection.simiPlaylists')" width="800px">
      <div v-if="simiLoading" class="flex items-center justify-center py-12">
        <SLoading class="text-3xl text-primary/70" />
      </div>
      <div v-else-if="simiPlaylists.length > 0">
        <CoverList :items="simiPlaylists" :virtual="false" :gap="16" @click="openSimiPlaylist" />
      </div>
      <div v-else class="flex items-center justify-center py-12 text-sm text-on-surface-variant/50">
        {{ t("collection.simiEmpty") }}
      </div>
      <template #footer="{ close }">
