...
/** 推荐内容：推荐歌单 / 雷达 / 歌手 / 新碟 / 排行榜 / 歌单广场 / 新歌速递 */
const {
  recommendPlaylists,
  recommendTitle,
  recommendSubtitle,
...
<template>
  <div class="h-full overflow-y-auto">
    ...
    >
      ...
      <!-- 排行榜 -->
      <section v-if="toplists.length > 0" class="flex flex-col gap-3">
        <div>
          <h3 class="text-lg font-semibold text-on-surface">{{ t("home.toplists.title") }}</h3>
          <p class="mt-0.5 text-xs text-on-surface-variant/50">{{ t("home.toplists.subtitle") }}</p>
        </div>
        <CoverList :items="toplists" :virtual="false" :gap="16" @click="openPlaylist" />
      </section>
      <!-- 歌单广场 -->
      <section v-if="playlistSquare.length > 0" class="flex flex-col gap-3">
        <div>
          <h3 class="text-lg font-semibold text-on-surface">{{ t("home.square.title") }}</h3>
          <p class="mt-0.5 text-xs text-on-surface-variant/50">{{ t("home.square.subtitle") }}</p>
        </div>
        <CoverList :items="playlistSquare" :virtual="false" :gap="16" @click="openPlaylist" />
      </section>
      <!-- 新歌速递 -->
      <section v-if="newSongs.length > 0" class="flex flex-col gap-3">
        <div>
          <h3 class="text-lg font-semibold text-on-surface">{{ t("home.newSongs.title") }}</h3>
          <p class="mt-0.5 text-xs text-on-surface-variant/50">{{ t("home.newSongs.subtitle") }}</p>
        </div>
        <div class="grid grid-cols-3 gap-3">
          <SCard
            v-for="(track, index) in newSongs"
            :key="`${track.source}:${track.id}`"
            radius="xl"
            size="small"
            hoverable
