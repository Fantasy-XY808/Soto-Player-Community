import type { Track } from "@shared/types/player";
import type { QualityLevel } from "@/utils/quality";
import { isPlatform } from "@shared/types/platform";
import { useStreamingStore } from "@/stores/streaming";
import { useSettingsStore } from "@/stores/settings";
import { useUserStore } from "@/stores/user";
import { clampQualityLevel } from "@/utils/quality";
import { resolveByPlugin } from "@/services/audioSource";
import { resolveNeteaseDownloadUrl } from "@/apis/song/netease";
import { resolveQQMusicUrl } from "@/apis/song/qqmusic";
import { resolveKugouUrl } from "@/apis/song/kugou";

/** 下载源解析结果 */
export interface DownloadSource {
  url: string;
  /** 已知格式（flac/mp3 等），用于扩展名 */
  format?: string;
  /** 已知体积（字节） */
  size?: number;
}

/**
 * 按下载音质解析歌曲下载地址
 * 三个平台都先走官方接口（匿名态拿 128k），失败 / 无版权再回落插件
 *
 * 权限校验：下载前必须 clamp 到用户实际可用的音质档位，
 * 防止越权下载 VIP / Hi-Res 等用户无权播放的音质
 *
 * @param track - 要下载的歌曲
 * @param level - 下载音质档位（会被 clamp 到用户权限范围内）
 * @returns 下载源；无法下载（VIP/试听/无插件/流媒体失败）返回 null
 */
export const resolveDownloadSource = async (
  track: Track,
  level: QualityLevel,
): Promise<DownloadSource | null> => {
  // 流媒体：音质由服务端决定，无需 clamp
  if (track.source === "streaming") {
    try {
      const url = await useStreamingStore().getStreamUrl(track, {
        playSessionId: crypto.randomUUID(),
      });
      return { url };
    } catch {
      return null;
    }
  }
  // 在线平台（netease/qqmusic/kugou）：先 clamp 到用户权限，避免越权下载
  const clampedLevel = isPlatform(track.source)
    ? (() => {
        const user = useUserStore();
        return clampQualityLevel(level, user.isLoggedIn, (user.profile?.vipType ?? 0) !== 0);
      })()
    : level;
  // 网易云官方接口（带格式与体积信息）
  if (track.source === "netease") {
    try {
      const usePlayback = useSettingsStore().system.download.usePlaybackForDownload;
      const resolved = await resolveNeteaseDownloadUrl(track, clampedLevel, usePlayback);
      if (resolved) return resolved;
    } catch {
      // 官方失败回落插件
    }
  }
  // QQ 音乐 / 酷狗官方接口：匿名态只返回 128k mp3，无格式/体积元数据
  if (track.source === "qqmusic" || track.source === "kugou") {
    try {
      const resolved =
        track.source === "qqmusic" ? await resolveQQMusicUrl(track) : await resolveKugouUrl(track);
      if (resolved) return { url: resolved, format: "mp3" };
    } catch {
      // 官方失败回落插件
    }
  }
  // 其他播放源或官方无果时回落插件（插件层内部也会 clamp）
  if (isPlatform(track.source)) {
    const res = await resolveByPlugin(track, clampedLevel);
    if (res.url) return { url: res.url };
  }
  return null;
};
