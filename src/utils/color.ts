...
/** 模块级：当前已写入 DOM 的封面色 RGB（补间起点） */
let currentCoverRgb: [number, number, number] = [...DEFAULT_COVER_RGB];
/** 进行中的补间 RAF ID（null 表示无） */
let coverTweenRafId: number | null = null;
...
/**
 * 封面色补间：RAF 驱动 RGB lerp，每帧写入 --s-cover
 * 所有 rgb(var(--s-cover)) 消费者随之获得 300ms 平滑过渡
 * @param from - 起点 RGB
 * @param to - 终点 RGB
 * @param startTime - 补间开始时间戳
 */
const tweenCoverColor = (
  from: [number, number, number],
  to: [number, number, number],
  startTime: number,
): void => {
  const elapsed = performance.now() - startTime;
  const t = Math.min(1, elapsed / COVER_TWEEN_DURATION);
  // ease-out cubic：起步快、收尾稳
  ...
  document.documentElement.style.setProperty("--s-cover", `${r} ${g} ${b}`);
  if (t < 1) {
    coverTweenRafId = requestAnimationFrame(() => tweenCoverColor(from, to, startTime));
  } else {
    currentCoverRgb = to;
    coverTweenRafId = null;
  }
};
...
const writeCoverColorWithTween = (targetHex: string | null): void => {
  ...
  // 颜色未变化：直接写入确保一致后返回
  if (
    currentCoverRgb[0] === targetRgb[0] &&
    currentCoverRgb[1] === targetRgb[1] &&
    currentCoverRgb[2] === targetRgb[2]
  ) {
  ...
  // 取消进行中的补间，从当前显示值出发
  if (coverTweenRafId !== null) {
    cancelAnimationFrame(coverTweenRafId);
    coverTweenRafId = null;
  }

  const from: [number, number, number] = [...currentCoverRgb];
  const startTime = performance.now();
  coverTweenRafId = requestAnimationFrame(() => tweenCoverColor(from, targetRgb, startTime));
};
