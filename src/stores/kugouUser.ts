import type { KugouProfile } from "@shared/types/kugou";
import type { KugouPlaylist } from "@/types/user";
...
export const useKugouUserStore = defineStore(
  ...
  () => {
    ...
    const fetchStatus = async (): Promise<boolean> => {
      try {
        const res = await window.api.kugou.fetchStatus();
        if (res.ok) {
    ...
    const logout = async (): Promise<void> => {
      await window.api.kugou.clearCookie();
      profile.value = null;
      likedSongIds.value = [];
      playlists.value = [];
