...
import { QUALITY_LABELS, clampQualityLevel, type QualityLevel } from "@/utils/quality";
...
/** 可下载音质档位（展示顺序：从高到低，含网易云 VIP 专属档位）
 * jymaster/sky/jyeffect 仅网易云 VIP 可下载；非 VIP 选了会被 clampQualityLevel 回落 */
const DOWNLOAD_QUALITY_LEVELS: QualityLevel[] = [
  "jymaster",
  "sky",
  "jyeffect",
  "hi-res",
...
export const useDownload = () => {
  ...
  const prepareRequest = async (
    ...
    // 在线平台先 clamp 到用户权限范围内，避免越权下载 VIP / Hi-Res 等音质
    const clampedLevel = isPlatform(track.source)
      ...
      : level;
    const source = await resolveDownloadSource(track, clampedLevel);
    if (!source) {
