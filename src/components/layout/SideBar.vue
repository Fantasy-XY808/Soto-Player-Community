...
const menuItems = computed<SMenuItem[]>(() => [
  ...
  { key: "/report", label: t("nav.musicReport"), icon: markRaw(IconLucideBarChart3) },
  // 发现
  { key: "divider-discover", type: "divider" },
  ...
  ...(systemSettings.listenTogether.enabled
    ? ([
        {
          key: "/listen-together",
          label: t("nav.listenTogether"),
          icon: markRaw(IconLucideUsers),
        },
      ] satisfies SMenuItem[])
    : []),
  // 我的歌单
