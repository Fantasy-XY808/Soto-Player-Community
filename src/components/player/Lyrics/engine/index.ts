...
export class LyricRenderer {
  ...
  private calculateFanLayout = (syncImmediate: boolean) => {
    ...
    for (let i = 0; i < lineCount; i++) {
      ...
      // 行 Y 位置：统一用累加的实际高度（滚动/非滚动同逻辑）
      const lineY = this.fanScrollLineY[i];
  ...
  private onAnimationFrame = (timestamp: number) => {
    ...
    // 三态：active 100% / 其余统一 inactiveAlpha；不再隐藏已播放行
    ...
    for (let i = 0; i < lineCount; i++) {
      ...
      if (isPassedHidden) {
        ...
        targetBright = isActive ? 1.0 : this.inactiveAlpha;
      }
  ...
  private handleWheel = (event: WheelEvent) => {
    ...
    const deltaY = this.lyricScrollDirection === "reverse" ? -event.deltaY : event.deltaY;
