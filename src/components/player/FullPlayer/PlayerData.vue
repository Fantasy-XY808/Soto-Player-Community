...
/** 网易云源优先显示用户选择档位（clamp 到权限内），其他源按物理参数推断 */
...
const qualityLabel = computed(() => {
  ...
  return getQualityLabel(media.detail?.quality);
});
...
const showLosslessIcon = computed(() => {
  if (canSwitchQuality.value) {
    ...
    return clamped === "hi-res" || clamped === "lossless" || clamped === "jymaster";
  }
  const level = getQualityLevel(media.detail?.quality);
  return level === "hi-res" || level === "lossless" || level === "jymaster";
});
