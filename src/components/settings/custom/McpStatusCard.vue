 ...
 * MCP 服务状态卡片
 *
 * 显示当前监听状态 + 端点地址，提供复制地址与重启服务按钮。
 * 端口 / 开关由 settings.aiIntegration 控制；服务实际状态由主进程推送。
 */
...
/** 当前 MCP 服务端点（监听中取实际端口，否则回退到配置端口预览） */
const address = computed(
  () => `http://127.0.0.1:${status.value.port ?? settings.system.aiIntegration.mcpPort}/mcp`,
);

/** 重启 MCP 服务（端口 / 开关变更后由用户主动触发） */
const restart = async (): Promise<void> => {
  if (!settings.system.aiIntegration.mcpEnabled || restarting.value) return;
  ...
  try {
    ...
    if (result.listening) {
      ...
      toast.error(
        t("settings.mcp.portInUse", { port: settings.system.aiIntegration.mcpPort }),
      );
...
<template>
  ...
  >
    ...
    <SButton
      ...
      variant="secondary"
      size="small"
      :disabled="!settings.system.aiIntegration.mcpEnabled"
      :loading="restarting"
      @click="restart"
    >
