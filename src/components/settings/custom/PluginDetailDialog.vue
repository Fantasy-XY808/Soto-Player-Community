...
/** 权限图标映射 */
const GRANT_ICONS: Record<"network" | "control" | "ui", Component> = {
  network: IconGlobe,
  control: IconGamepad,
  ui: IconMenu,
};
...
/**
 * 由 manifest.type 推导权限列表
 * - 音源类（type !== "control"）：network 自动授予（解析播放 URL 必需）
 * - 控制类（type === "control"）：声明 control + ui
 */
const grantRows = computed(() => {
  const isSource = props.info?.manifest.type !== "control";
  const grants = isSource
    ? (["network"] as const)
    : (["control", "ui"] as const);
  return grants.map((key) => ({
    key,
...
const hasGrant = computed(() => grantRows.value.length > 0);

/** ready 时的状态对象（含 sources/events/controls/settings） */
    ...
    <div v-if="info" class="flex flex-col gap-5">
      ...
      <div class="flex flex-col gap-2">
        <div class="flex items-center gap-2">
          ...
          <STag size="small" round type="default" variant="soft">
